...
export async function checkQuestion(req, res) {
    const { selectedList, clientUrl } = req.body;
    const targetList = new ObjectId(selectedList);

    const { listsCollection } = res.locals.db;
    const { questions } = await listsCollection.findOne({ _id: targetList });

    for (let i = 0; i < questions?.length; i++) {
        if (questions[i].url === clientUrl) {
